...
impl Scanner {
    ...
    fn read_character(&mut self) {
        ...
        println!(
            ...
            char::from(self.current_character)
        );
    ...
    pub fn next_tok(&mut self) -> Tk {
        ...
        let tok = match self.current_character {
            ...
            0 => Tk::EOF,
            240..=244 | 224..=239 => {
                //? basic 3-bytes and 4-byte emojis
                //? maybe use a library❓
                if self.current_character >= 240 && self.current_character <= 244 {
                    //? Four-byte emoji
                    let start = self.current_position;
                    self.read_character();
                    self.read_character();
                    ...
                    Tk::EMOJI(emoji, self.current_line)
                } else if self.current_character >= 224 && self.current_character <= 239 {
                    //? Three-byte emoji
                    let start = self.current_position;
                    self.read_character();
                    self.read_character();
