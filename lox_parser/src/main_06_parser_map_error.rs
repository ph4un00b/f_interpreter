...
impl fmt::Display for Expr {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Expr::None => write!(f, "❌ algún map error!")?,
        }
...
impl Parser {
    ...
    fn equality(&mut self) -> Result<Expr, String> {
        let mut expr = self.comparison().map_err(|x| format!("equality1:{x}"))?;
        while self.current_token == Tk::EQ {
            self.consume_token();
            let operator = self.prev_token.clone();
            let right = self.comparison().map_err(|x| format!("equality2:{x}"))?;
    ...
    fn comparison(&mut self) -> Result<Expr, String> {
        let mut expr = self.term().map_err(|x| format!("comparison1:{x}"))?;
        while self.current_token == Tk::LT {
            self.consume_token();
            let operator = self.prev_token.clone();
            let right = self.term().map_err(|x| format!("comparison2:{x}"))?;
            expr = Expr::binary_op(expr, operator, right);
        }
        Ok(expr)
    }
    ...
    fn term(&mut self) -> Result<Expr, String> {
        let mut expr = self.factor().map_err(|x| format!("term1:{x}"))?;
        while self.current_token == Tk::Sub || self.current_token == Tk::Mul {
            self.consume_token();
            let operator = self.prev_token.clone();
            let right = self.factor().map_err(|x| format!("term2:{x}"))?;
            expr = Expr::binary_op(expr, operator, right);
        }
        Ok(expr)
    }
    ...
    fn factor(&mut self) -> Result<Expr, String> {
        let mut expr = self.unary().map_err(|x| format!("factor1:{x}"))?;
        while self.current_token == Tk::Mul {
            self.consume_token();
            let operator = self.prev_token.clone();
            let right = self.unary().map_err(|x| format!("factor2:{x}"))?;
            expr = Expr::binary_op(expr, operator, right);
        }
        Ok(expr)
    }
    ...
    fn unary(&mut self) -> Result<Expr, String> {
        if self.current_token == Tk::Bang || self.current_token == Tk::Sub {
            self.consume_token();
            let operator = self.prev_token.clone();
            let right = self.unary().map_err(|x| format!("unary:{x}"))?;
    ...
    fn primary(&mut self) -> Result<Expr, String> {
        match self.current_token {
            ...
            Tk::Lpar => {
                self.consume_token();
                let expr = self.expression().map_err(|x| format!("primary:{x}"))?;
