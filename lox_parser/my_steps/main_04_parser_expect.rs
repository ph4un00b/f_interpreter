...
impl Parser {
    ...
    fn parse(&mut self) -> Expr {
        ...
        self.expression().unwrap_or_else(|_| {
            println!("❌ unwrap_or_else error desde parse!");
            Expr::None
        })
    }
    ...
    fn equality(&mut self) -> Result<Expr, String> {
        let mut expr = self.comparison().expect("equality, expected EXPRESSION!");
        while self.current_token == Tk::EQ {
            self.consume_token();
            let operator = self.prev_token.clone();
            let right = self.comparison().expect("equality, expected EXPRESSION!");
    ...
    fn comparison(&mut self) -> Result<Expr, String> {
        let mut expr = self.term().expect("comparison, expected EXPRESSION!");
        while self.current_token == Tk::LT {
            self.consume_token();
            let operator = self.prev_token.clone();
            let right = self.term().expect("comparison, expected EXPRESSION!");
            expr = Expr::binary_op(expr, operator, right);
        }
        Ok(expr)
    }
    ...
    fn term(&mut self) -> Result<Expr, String> {
        let mut expr = self.factor().expect("term, expected EXPRESSION!");
        while self.current_token == Tk::Sub || self.current_token == Tk::Mul {
            self.consume_token();
            let operator = self.prev_token.clone();
            let right = self.factor().expect("term, expected EXPRESSION!");
            expr = Expr::binary_op(expr, operator, right);
        }
        Ok(expr)
    }
    ...
    fn factor(&mut self) -> Result<Expr, String> {
        let mut expr = self.unary().expect("factor, expected EXPRESSION!");
        while self.current_token == Tk::Mul {
            self.consume_token();
            let operator = self.prev_token.clone();
            let right = self.unary().expect("factor, expected EXPRESSION!");
            expr = Expr::binary_op(expr, operator, right);
        }
        Ok(expr)
    }
    ...
    fn unary(&mut self) -> Result<Expr, String> {
        if self.current_token == Tk::Bang || self.current_token == Tk::Sub {
            self.consume_token();
            let operator = self.prev_token.clone();
            let right = self.unary().expect("unary, expected EXPRESSION!");
    ...
    fn primary(&mut self) -> Result<Expr, String> {
        match self.current_token {
            ...
            Tk::Lpar => {
                self.consume_token();
                //? aquí podría usarse esta versión
                //? por que podría capturar un error profundo
                //? @see https://rust-lang.github.io/rfcs/3137-let-else.html#else-block-fall-back-assignment
                //? let Ok(mut expr) = self.expression() else match {
            ...
            _ => Err(string_err(
                ...
                "Expected expression in #primary",
            )),
        }
    }
