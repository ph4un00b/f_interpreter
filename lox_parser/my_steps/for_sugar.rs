...
#[allow(dead_code)]
impl<'a> Env<'a> {
    fn new(enclosing: Option<&'a Env<'a>>) -> Self {
        Self {
            enclosing,
            values: HashMap::new(),
        }
...
impl Interpreter {
    ...
    fn eval_block(&mut self, statements: Vec<Statement>, local_env: Env) {
        ...
        // let previous_env = std::mem::replace(&mut self.global_env, local_env);
         ...
         */
        println!("prev: {:#?}", previous_env);
        println!("global: {:#?}", self.global_env);
    ...
    fn eval_statement(&mut self, state: Statement) {
        match state {
            Statement::While { condition, body } => {
                ...
                let Ok(evaluated_condition) = maybe_condition else { todo!() };
                // println!("ev: {:?}", evaluated_condition);
                while evaluated_condition == V::Bool(true) {
            ...
            Statement::Block(statements) => {
                println!("current-global: {:#?}", self.global_env);
                // self.eval_block(
                //     statements,
                ...
                // )
                let previous_env = self.global_env.clone();
                self.global_env = Env::new(Some(Box::new(self.global_env.clone())));
                for state in statements {
                    self.eval_statement(state);
                }
                // println!("prev: {:#?}", previous_env);
            ...
            Statement::Expr(expr) => {
                // println!("ex-st: {:?}", expr);
                let maybe_val = self.eval_expr(expr);
                let Ok(_evaluated_condition) = maybe_val else { todo!() };
            }
    ...
    fn eval_expr(&mut self, expr: Expr) -> RValue {
        ...
        match expr {
            Expr::Assign(to_identifier, right) => {
                ...
                //? var a = 1;
                // todo: print a = 2; // "2".
                println!("ass: {:?} = {:?}", to_identifier, value);
                let x = self.global_env.assign(to_identifier, value)?;
                println!("x {:?}", x);
                Ok(x)
            }
            Expr::Let(identifier) => {
                let x = self.global_env.get(identifier)?;
                println!("let {:?}", x);
                Ok(x)
            }
            Expr::Unary { operator, right } => {
